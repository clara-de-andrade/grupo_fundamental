...
\begin{equation*}
\begin{gathered}
    \text{Para todo $\varepsilon > 0$, existe $\delta > 0$ tal que, para todo $\phi \in [0, 2\pi]$,}
    \\
    \text{se $0 < ||\gamma(\theta) - \gamma(\phi)|| < \delta$ e $t \in I$ }
    \text{então $||\eta_{\theta}(t) - \eta_{\phi}(t)|| < \varepsilon$.}
\end{gathered}
\end{equation*}
%
Ou seja, os pontos $\gamma(\phi)$ que estão em uma vizinhança de $\gamma(\theta)$ em $S^1$ são associados a curvas $\eta_{\phi}$ cujos pontos $\eta_{\phi}(t)$ também devem estar em uma vizinhança dos pontos $\eta_{\theta}(t)$ em $\eta_\theta$ para todo $t \in I$. Contudo, é impossível que todos os pontos ``próximos'' de $\gamma(\pi) = -p$ estejam associados a curvas ``próximas'' de $\eta_\pi$, neste sentido, sem que elas interceptem o interior de $S^1$. Note que isto é consistente com a continuidade de $\eta$ em $\overline{B^2}$, pois $\overline{B^2}$ contém o seu interior; contudo, $S^1$ não. Portanto, como uma homotopia em $S^1$, $\eta$ deve possuir uma descontinuidade, o que é uma contradição.%
\footnote
{
    {\it (Resumo da prova)} Assuma sem perda de generalidade que $\eta_\pi$ está contida no plano $y \geq 0$. Fixe $0 < \varepsilon < 1$ e seja $\delta > 0$ dado pela continuidade de $\eta$, na forma enunciada acima. Por fim, seja $\phi \in (\pi, \pi + \delta)$ tal que $\gamma(\phi)$ está contido no plano $y < 0$. Existe $t' \in I$ tal que $\eta_\pi(t') = (0, 1)$, e como $||\eta_\pi(t') - \eta_\phi(t')|| < 1$ então $\eta_\phi(t')$ pertence ao plano $y > 0$. Pelo Teorema do Valor Intermediário, existe $t_0 \in (0, t')$ tal que $\eta_\phi(t_0)$ intercepta a reta $y = 0$. Por fim, temos que $||\eta_\phi(t_0)|| < 1$. \qed
}

Essas observações dependem crucialmente do fato de que $S^1$ não contém o seu próprio interior. Se considerarmos espaços homeomórficos a $S^1$, como $\overline{B^2} \setminus \{ (0, 0) \}$, por possuírem essa propriedade, obtemos o mesmo resultado: não existem homotopias entre as curvas $\gamma$ e $\msf{const}_p$. Neste sentido, a existência ou não destas homotopias capturam formalmente o fato, visualmente evidente nesses casos particulares, de que $S^1$, $\overline{B^2} \setminus \{ (0, 0) \}$ possuem ``furos'', correspondendo às regiões onde ocorrem as descontinuidades no seu interior, enquanto $\overline{B^2}$ não possui ``buracos''. De fato, uma motivação histórica para o conceito de homotopia é determinar a existência desses ``furos'' em certos espaços topológicos (e em particular, em superfícies como $S^n$ e $\overline{B^{n + 1}}$).%
\linebreak\vspace{-1.2em}